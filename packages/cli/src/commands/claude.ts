...
export function resolveAdoptableLocalBackendSessionId(options: {
  ...
  // close to the Inkwell session start time, adopt it as the backend linkage.
...
export function shouldAutoResumeRuntimeSession(
  ...
  //     2) resuming a tracked Inkwell session, or
...
async function resolvePcpAuthEnv(verbose: boolean): Promise<Record<string, string>> {
  ...
  if (verbose) console.log(chalk.dim('Inkwell auth: not authenticated'));
  return {};
}
...
function buildInjectedStartupContext(
  ...
  const sections: string[] = [
    ...
    '_Use this as startup context; if stale, call `mcp__inkwell__bootstrap` manually._',
  ];
...
async function resolveCodexStartupContextBlock(options: {
  ...
  try {
    ...
    if (verbose)
      console.log(chalk.dim('Inkwell startup context: injected into model_instructions_file'));
...
/**
 * Resolve studioId from identity.json, falling back to an Inkwell API lookup
...
export function filterPcpSessionsForContext(
  ...
  for (const session of backendMatched) {
    ...
    // Inkwell sessions (before hooks/close persist workingDir/backendSessionId) are
...
function printPcpUnavailableWarning(reason: string, cwd = process.cwd()): void {
  ...
  if (!existsSync(mcpPath)) {
    ...
    try {
      ...
      if (!parsed.mcpServers?.inkwell) {
        console.log(chalk.yellow('  .mcp.json is missing mcpServers.inkwell.'));
      }
    } catch {
      console.log(chalk.yellow('  .mcp.json could not be parsed.'));
    }
  }
  console.log(chalk.dim('  To reconnect Inkwell features:'));
...
async function ensurePcpSessionContext(
  ...
  if (process.stdin.isTTY || options.listCandidates) {
    ...
    if (!hooks.installed) {
      console.log(
        chalk.yellow(
          `\n⚠ Inkwell hooks not installed for ${backend} (${hooks.configPath}). Session mapping may be unreliable.`
        )
      );
  ...
  if (options.listCandidates || options.listCandidatesJson) {
    ...
    if (options.listCandidatesJson) {
      ...
      console.log(
        chalk.dim(
          `\n  Local sessions: ${localCandidates.length} (${selectableLocals} selectable, ${localCandidates.length - selectableLocals} already linked to Inkwell)`
        )
      );
      console.log('');
    }
  ...
  // - Codex: Sessions are independent local files. Inkwell links are advisory.
  //   Drops pre-linked sessions on new Inkwell session to avoid stale links,
  ...
  // use it as the resume target even if we created a new Inkwell session.
  ...
  if (verbose) {
    console.log(chalk.dim(`Inkwell session: ${chosen.id}`));
...
export async function runClaude(
  ...
  if (!agentId) {
    ...
    console.error(
      chalk.dim(
        'Run `ink init` to set up Inkwell in this repo, or `ink awaken` to create a new SB.'
      )
    );
