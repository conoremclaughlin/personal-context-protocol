...
export function resolveAdoptableLocalBackendSessionId(options: {
  ...
  // created a brand-new Inkstand session from the picker. "Start new session" must
  ...
  // close to the Inkstand session start time, adopt it as the backend linkage.
...
export function shouldAutoResumeRuntimeSession(
  ...
  //     2) resuming a tracked Inkstand session, or
...
async function resolvePcpAuthEnv(verbose: boolean): Promise<Record<string, string>> {
  try {
    ...
    if (token) {
      if (verbose) console.log(chalk.dim('Inkstand auth: token injected'));
      return { INK_ACCESS_TOKEN: token };
    }
  } catch {
    // Silently skip — auth is best-effort
  }
  if (verbose) console.log(chalk.dim('Inkstand auth: not authenticated'));
  return {};
}
...
function buildInjectedStartupContext(
  ...
  const sections: string[] = [
    '_Generated by `ink` at session start from Inkstand `bootstrap`._',
    '_Use this as startup context; if stale, call `mcp__inkstand__bootstrap` manually._',
  ];

  // Session identity — always-available IDs for debugging and routing verification
  if (sessionIds?.pcpSessionId) {
    const idParts: string[] = [`- Inkstand Session: \`${sessionIds.pcpSessionId}\``];
...
async function resolveCodexStartupContextBlock(options: {
  ...
  try {
    ...
    if (verbose)
      console.log(chalk.dim('Inkstand startup context: injected into model_instructions_file'));
    ...
    if (verbose) console.log(chalk.dim('Inkstand startup context: unavailable (continuing)'));
    return undefined;
  }
}
...
/**
 * Resolve studioId from identity.json, falling back to an Inkstand API lookup
...
async function resolveStudioId(cwd: string): Promise<{
  ...
  // No UUID in identity.json — try resolving via Inkstand API by worktree path
  try {
...
export function filterPcpSessionsForContext(
  ...
  for (const session of backendMatched) {
    ...
    // Inkstand sessions (before hooks/close persist workingDir/backendSessionId) are
...
export function resolveBackendSessionSeedId(options: {
  ...
  // Seed Claude session ID only on first run when Inkstand session is created now.
...
export function getCodexLocalSessionsForProject(
  ...
  // still attachable debugging artifacts, and hiding them regressed `ink -b codex`
...
function printPcpUnavailableWarning(reason: string, cwd = process.cwd()): void {
  ...
  console.log(chalk.yellow(`\n⚠ Inkstand session service unavailable (${reason}).`));
  ...
  if (!existsSync(mcpPath)) {
    ...
    try {
      ...
      if (!parsed.mcpServers?.inkstand) {
        console.log(chalk.yellow('  .mcp.json is missing mcpServers.inkstand.'));
      }
    } catch {
      console.log(chalk.yellow('  .mcp.json could not be parsed.'));
    }
  }
  console.log(chalk.dim('  To reconnect Inkstand features:'));
  console.log(chalk.dim(`    curl -sS ${serverUrl.replace(/\/+$/, '')}/health`));
  console.log(chalk.dim('    ink auth login'));
  console.log(chalk.dim('    ink init'));
...
async function ensurePcpSessionContext(
  ...
  if (process.stdin.isTTY || options.listCandidates) {
    ...
    if (!hooks.installed) {
      console.log(
        chalk.yellow(
          `\n⚠ Inkstand hooks not installed for ${backend} (${hooks.configPath}). Session mapping may be unreliable.`
        )
      );
  ...
  // Keep the picker de-duplicated: linked locals are rendered through the Inkstand row
  ...
  const startNewPcpSession = async (): Promise<PcpSessionSummary | undefined> => {
    ...
    try {
      ...
      // Backward compatibility: older Inkstand servers may reject the newer `sessionId`
      // parameter. Retry once without it so "start new session" still creates a
      // real server-side Inkstand session instead of a synthetic local-only UUID.
  ...
  if (options.listCandidates || options.listCandidatesJson) {
    ...
    if (options.listCandidatesJson) {
      ...
      console.log(
        chalk.dim(
          `\n  Local sessions: ${localCandidates.length} (${selectableLocals} selectable, ${localCandidates.length - selectableLocals} already linked to Inkstand)`
        )
      );
      console.log('');
    }
  ...
  if (normalizedSelectionOverride) {
    ...
    if (selection === 'new' || selection === '__new__') {
      ...
      if (selectedLocalBackendSessionId && pcpAvailable) {
        // Find-or-link: prefer an existing Inkstand session linked to this backend
        // session, then try an unlinked Inkstand session in the same studio, and
        ...
        if (linkedPcpSession) {
          ...
          // Try to find an unlinked Inkstand session for this agent+studio that
    ...
    for (const session of activeSessions) {
      ...
      const sourceLabel = showOwner ? `Inkstand/${ownerLabel}` : 'Inkstand';
    ...
    try {
      ...
      if (selection === '__new__') {
        ...
        if (selectedLocalBackendSessionId && pcpAvailable) {
          // Find-or-link: prefer linked Inkstand session, then adopt unlinked, then create new
          const linkedPcpSession = pcpSessionByBackendSessionId.get(selectedLocalBackendSessionId);
          if (linkedPcpSession) {
            chosen = linkedPcpSession;
          } else {
  ...
  // - Codex: Sessions are independent local files. Inkstand links are advisory.
  //   Drops pre-linked sessions on new Inkstand session to avoid stale links,
  ...
  // use it as the resume target even if we created a new Inkstand session.
  ...
  if (staleTrackedBackendSessionId && process.stdin.isTTY) {
    if (fallbackMode === 'resume_pcp_session_id' && chosen.id) {
      console.log(
        chalk.yellow(
          `\nLinked Claude session ${staleTrackedBackendSessionId.slice(0, 8)} is unavailable for this project; retrying with Inkstand-linked Claude session ${chosen.id.slice(0, 8)}.`
        )
      );
    } else {
      const backendLabel = backend[0].toUpperCase() + backend.slice(1);
  ...
  if (verbose) {
    console.log(chalk.dim(`Inkstand session: ${chosen.id}`));
...
export async function runClaude(
  ...
  if (!agentId) {
    ...
    console.error(
      chalk.dim('Run `ink init` to set up Inkstand in this repo, or `ink awaken` to create a new SB.')
    );
